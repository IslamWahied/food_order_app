...
import 'package:food_order_app/old/modules/customer/home/home_screen.dart';
...
class RestrantListScreen extends StatelessWidget {
  const RestrantListScreen({required Key key}) : super(key: key);
        ...
        child: BlocConsumer<HomeCubit, HomeState>(
          builder: (context, state) {
           ...
           return   BackdropScaffold(
                   ...
                   children: [
                     GestureDetector(
                       onTap: () {
                         cubit.isShowBackLayer = false;
                         NavigatToAndReplace(
                             context, const HomeLayout(key:Key("30")));
                     ...
                     IconButton(
                         onPressed: () {
                           cubit.isShowBackLayer = false;
                           NavigatToAndReplace(
                               context, const HomeLayout(key:Key("27")));
             ...
             backLayer: UserBackLayerMenu(key: const Key("121")),
...
Widget buildGridProjects(Project model, context) => GestureDetector(
      onTap: () {
        ...
        Navigator.of(HomeCubit.get(context).mAnavigatorkey.currentContext!).push(
            MaterialPageRoute(
                builder: (BuildContext context) => const HomeScreen(key:Key("40"))));
